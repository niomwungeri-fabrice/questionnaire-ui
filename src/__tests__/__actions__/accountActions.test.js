...
describe('Non-Async Account Actions', () => {
  it('should create an action setError ', () => {
    ...
    expect(handleInputs({ ...testInput })).toEqual(expectedAction);
  });
});
...
describe('Async Account Actions', () => {
  ...
  it('should fail to login', () => {
    ...
    return store.dispatch(handleSignIn(validUserPayload)).then(() => {
      const actions = store.getActions();
      expect(actions).toEqual(expectedResults);
    });
  });
  ...
  it('should login successfully', () => {
    ...
    return store.dispatch(handleSignIn(invalidUserPayload)).then(() => {
      const actions = store.getActions();
      expect(actions).toEqual(expectedResults);
    });
  });
  it('should  set current user successfully', () => {
    ...
    return store.dispatch(handleCurrentAccount(fakeTokenAccess)).then(() => {
      const actions = store.getActions();
      expect(actions).toEqual(expectedResults);
    });
  });
